...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ content, onUseAsInput, onImageClick, originalImageUrl }) => {
  ...
  const handleDownloadComparison = useCallback(() => {
    if (!originalImageUrl || !content.imageUrl) return;
    ...
    const loadOriginal = new Promise(resolve => originalImg.onload = resolve);
    const loadGenerated = new Promise(resolve => generatedImg.onload = resolve);

    originalImg.src = originalImageUrl;
    generatedImg.src = content.imageUrl;

    Promise.all([loadOriginal, loadGenerated]).then(() => {
      canvas.width = originalImg.width + generatedImg.width;
      canvas.height = Math.max(originalImg.height, generatedImg.height);
      
      // Fill background in case images are different sizes
      ctx.fillStyle = '#000000';
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      ctx.drawImage(originalImg, 0, (canvas.height - originalImg.height) / 2);
      ctx.drawImage(generatedImg, originalImg.width, (canvas.height - generatedImg.height) / 2);

      const link = document.createElement('a');
      link.href = canvas.toDataURL('image/png');
      link.download = `comparison-image-${Date.now()}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    });
